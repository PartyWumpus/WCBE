...
fn hrti_g(this: &mut Cursor, _state: &mut StateTempName, _settings: &Settings) {
    // timer granularity in microseconds
    this.push(1);
}

fn hrti_m(this: &mut Cursor, state: &mut StateTempName, _settings: &Settings) {
    state.hrti_marks.insert(this.id, Instant::now());
}

fn hrti_t(this: &mut Cursor, state: &mut StateTempName, _settings: &Settings) {
...
fn hrti_e(this: &mut Cursor, state: &mut StateTempName, _settings: &Settings) {
    state.hrti_marks.remove(&this.id);
}

fn hrti_s(this: &mut Cursor, state: &mut StateTempName, _settings: &Settings) {
